...
output_json = f'output_val_{date}.json'
...
def resJson(testImg):
    ...
    for i, img_name in enumerate(testImg):
        im = Image.open(f"./HW1_2024_dataset/valid2017/images/{img_name}")
...
def plot_results(pil_img, prob, boxes):
    ...
    plt.axis("off")
    plt.savefig(f"./inference_test_{date}.jpg")
    # plt.show()
...
if __name__ == "__main__":
    ...
    # 獲取測試圖像
    testImg = get_filenames_in_directory("./HW1_2024_dataset/valid2017/images")

    # 運行推理
    resJson(testImg)

    # 可視化一個示例（可選）
    im = Image.open("./HW1_2024_dataset/valid2017/images/pexels-photo-9077986.jpeg")
    scores, boxes = detect(im, detr, transform)
